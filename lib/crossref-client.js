...
/**
 * 非研究文章排除规则（内置，避免文件系统依赖）
 */
const EXCLUSION_RULES = {
  titlePatterns: [
    "issue information", "table of contents", "editorial board",
    "announcement", "call for papers", "book review", "corrigendum",
    "erratum", "addendum", "retraction", "editorial", "preface",
    "foreword", "acknowledgment", "in memoriam", "in memory of",
    "editor's note", "guest editorial", "index", "supplement"
  ],
  subtypes: [
...
export function isValidResearchArticle(item, journalConfig) {
  ...
  if (subtype && EXCLUSION_RULES.subtypes.includes(subtype.toLowerCase())) {
    return false;
  }

  // 2. 检查标题模式
  const title = (item.title?.[0] ?? "").toLowerCase().trim();
  for (const pattern of EXCLUSION_RULES.titlePatterns) {
